...
class BallTracker(Node):
    ...
    def process_image(self, msg):
        ...
        # _, self.img_width, _ = self.cv_image.shape
        moments = cv2.moments(self.binary_image)
        if moments['m00'] != 0:
            self.center_x, self.center_y = moments['m10']/moments['m00'], moments['m01']/moments['m00']
            print("updated center: ",self.center_x)

        self.center_x = self.center_x /self.img_width - 0.5
    ...
    def run_loop(self):
        ...
        if not self.cv_image is None:
            self.binary_image = cv2.inRange(self.cv_image, (self.red_lower_bound,self.green_lower_bound,self.blue_lower_bound), (self.red_upper_bound,self.green_upper_bound,self.blue_upper_bound))
    ...
    def run(self):
        msg = Twist()
        if self.center_x < 0:
            msg.angular.z = -self.center_x * self.kp
        elif self.center_x > 0:
            msg.angular.z = self.center_x * self.kp
        msg.linear.x = .5
        print('turing at: ', msg.angular.z)
