...
### Learning Goal
I wanted to learn more about machine leanrning, especially computer vision. Therefore this project was more focused on application of computer vision rather than robot interaction like in previous projects. Computer vision neural networks are large and hard to train because of the need of huge database. Therefore I utilized transfer learning with various available pretained models and different preprocessing image.

### Data Preparation
I used onboard camera from neato and webcam from laptop with openCV to record videos with different hand gestures and save the frames as the training, validation, and test datasets.
#### Image Recording
...
##### Data Augmentation
The most crucial one was to use data augmentation to reduce overfitting and increase sample diversity for this small size dataset. I added random rotation and vertical flip to the dataset. (There was no horizontal flip as normal data augmentation because it would confuse the left/right category).
...
We use a pretrained model with classification ability and make it learn to classify images of our own classes. Transfer learnings are widely used in the field of computer vision (especially image classification), natural language processing, and speech recognition. They could boost performance for small dataset with specific tasks.

#### Base Model
I tried using MobileNet_v3, inception_v3, inception_v2_resnet as base models and inception_v2_resnet produced the best result using the same datasets. This might be because this model has the largest network. 
...
In model training, I noticed that with too few layers (50 out of 300 layers with around 50 trainable variables) would make the training time much quicker but lower accuracy. However, too many trainable layers (200 layers with 150+ trainable variables) significantly slows down the training with bad output because we will be overfitting (high training accuracy) the whole neural network with changing so many output. As a result, keeping 
both the trainable layer number and variables around 100 yeilds a balanced result.
...
## Challenges
### 1. Enviroment Setup
Google colab was not fast and stable enough for thousands of images. However, setting up GPU enviroment on an intel mac could be painful. Luckily I will not have to do this again in the future.
### 2. Constant Zero Loss
At the first several trainings, I kept getting zero loss from both training and validation with ~60% accuracy. It took me a tremendous amount of time from useless training and trouble shooting. From learning through parameters of `image_dataset_from directory` and different types of loss functions, I fixed this issue by setting `label_mode` to `int` and use sparse categorical cross entrophy. 


## Key Takeaways
### 1. Transfer Learning
I was able to use tensorflow pretrained model APIs for transfer learning which was really exciting and I realized they are extremely useful in computer vision because they could perform really well even if they were not for some specific tasks.
...
4. Loss functions: 
	Loss functions is how we measure how far is our prediction to the correct output. Our weights for each neturon is determined through the process of lowering loss. There are many types of loss functions and they each serve different modeling decisions and I was only able to 	learn about most used types:
	- Mean Squared Error: This is most used in regression probelms where we need the output to be a continuous value.
	- Binary Crossentropy: 
...
5. Activation functions:
	...
	- **Sigmoid**:  $f(x) = \frac{1}{1+e^{-z}}$
		...
		-  Drawbacks: 
			...
			- It has vanishing gradient problem. (See ReLu part.)
	- **Softmax**: This is used in **multiple label classification** because it calculates the probability of N different events with their sum equals to 1. This would make sure all class possibilities are mutually exclusive.
	- **ReLU** (Rectified Linear Unit): $f(x) = \text{max}(0,x)$
		- It is the most used activation funciton in **hidden layers** of MLP and CNN models because it is not only fast but also solves the vanishing gradient problem. 
